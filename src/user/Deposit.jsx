...
const Deposit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosClient.post('/transactions/deposit', null, {
        params: {
      ...
      setSuccess(res.data || 'Deposit successful!');
      ...
      console.error(err);
      setError(err.response?.data?.message || err.message || 'Deposit transaction failed.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="container" style={{ maxWidth: '600px' }}>
      <div className="card shadow-sm border-0 mt-4">
        <div className="card-header bg-success text-white py-3">
          <h4 className="mb-0 fw-bold">📥 Deposit Funds</h4>
        </div>
        <div className="card-body p-4">
          ...
          <form onSubmit={handleSubmit} noValidate className={validated ? 'was-validated' : ''}>
            ...
            <div className="mb-4">
              <label htmlFor="amount" className="form-label">Deposit Amount (₹)</label>
              <input
                type="number"
                id="amount"
                name="amount"
